...
function renderPage() {
  ...
  document.querySelector('main').innerHTML = route === 'home' ? homeMarkup : (route === 'listings' ? listingsPage : (route.startsWith('listing-') ? listingDetailPage : (route.startsWith('product-') ? productDetailPage : (route.startsWith('category-') ? categoryPage(route.replace('category-', '').replaceAll('-', ' ')) : (pages[route] || pages.catalog)))))

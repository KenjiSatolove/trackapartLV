...
// Every one of these values can originate from an anonymous or ordinary
// signed-up user (checkout form, listing submission, review, profile edit)
// and gets interpolated into innerHTML template strings below. Without this,
// e.g. a listing title of `<img src=x onerror=fetch(...document.cookie)>`
// executes in whichever browser renders it next — including the admin's,
// the moment they open the moderation queue or orders tab.
...
function productCardMarkup(product, index, capped) {
  ...
  return `<article class="product-card${capped ? ' card-capped' : ''}" data-product-id="${product.id}" data-name="${escapeHtml(dataName)}" data-price="${product.price}" data-category="${escapeHtml((product.category || '').toLowerCase())}" data-brand="${escapeHtml((product.brand || '').toLowerCase())}" data-model="${escapeHtml((product.model || '').toLowerCase())}" data-order="${index ?? 0}"><div class="product-image"><img src="${product.image || ''}" alt="${escapeHtml(product.name)}"/><span class="product-tag">${escapeHtml(product.tag || product.condition || '')}</span><button class="quick-add" data-id="${product.id}" aria-label="Pievienot grozam">+</button></div><div class="product-meta"><span>${escapeHtml(product.type)}</span><small>${escapeHtml(product.code || '')}</small></div><h3>${escapeHtml(product.name)}</h3><div class="product-bottom"><strong>${Number(product.price).toFixed(2).replace('.', ',')} €</strong><button class="add-text" data-id="${product.id}">PIEVIENOT <span>↗</span></button></div></article>`
}
...
function homeMarkup() {
  return `
    ...
    <section class="search-section reveal" id="catalog-preview"><div class="section-kicker">01 / ATRODI SAVU DETAĻU</div><div class="search-heading"><h2>Ko tu meklē?</h2><p>Meklē pēc nosaukuma, OEM koda vai detaļas numura.</p></div>${searchBoxMarkup()}</section>
    ...
    <section class="product-section reveal" id="new-arrivals"><div class="section-top"><div><div class="section-kicker">03 / JAUNUMI NOLIKTAVĀ</div><h2>Pēdējie <em>atradumi.</em></h2></div><a class="text-link" href="#listings">SKATĪT VISUS <span>↗</span></a></div>${productGridMarkup(products.slice(0, 8))}</section>

    <section class="listings-section reveal" id="listings-preview"><div class="section-top"><div><div class="section-kicker">04 / KOPIENAS SLUDINĀJUMI</div><h2>Ko pārdod<br><em>citi.</em></h2></div><a class="text-link" href="#sell">PĀRDOT SAVU DETAĻU <span>↗</span></a></div><div class="listing-table" id="listing-table"><div class="listing-head"><span>DETAĻA</span><span>AUTO</span><span>STĀVOKLIS</span><span>CENA</span><span></span></div><p class="listing-loading">Ielādējam sludinājumus...</p></div></section>
...
function renderCart() {
  ...
  items.innerHTML = cartItems.length ? cartItems.map((product) => `<div class="cart-item"><div><strong>${escapeHtml(product.name)}</strong><small>${escapeHtml(product.code || '')}</small></div><b>${Number(product.price).toFixed(2).replace('.', ',')} €</b></div>`).join('') : '<p id="cart-empty">Grozs ir tukšs.</p>'
  primeAndTranslate()
}
...
document.querySelector('#checkout-button').addEventListener('click', () => {
  ...
  document.querySelector('#checkout-form').addEventListener('submit', async (event) => {
    ...
    checkoutView.innerHTML = `<div class="section-kicker">PALDIES!</div><h2>Pasūtījums <em>saņemts.</em></h2><p class="form-message">Pasūtījums #${order.id} saņemts. Mēs sazināsimies pa e-pastu ${escapeHtml(data.get('email'))}, lai vienotos par apmaksu un piegādi.</p><button class="text-button" type="button" id="checkout-close">AIZVĒRT</button>`
...
function renderPage() {
  ...
  document.querySelectorAll('.main-nav a, .mobile-nav-links a, .mobile-nav-legal a').forEach((link) => link.classList.toggle('active', link.getAttribute('href') === `#${route}`))
...
function adminProductFormMarkup(product) {
  const p = product || {}
  return `<form class="admin-product-form" id="admin-product-form"><h3>${product ? 'Edit product' : 'Add product'}</h3><input type="hidden" name="id" value="${p.id || ''}"><label>NAME<input name="name" required value="${escapeHtml(p.name || '')}"></label><div class="form-two"><label>CATEGORY<select name="category">${CATEGORIES.map((c) => `<option value="${c[1] === 'Riteņi & diski' ? 'Riteņi un diski' : c[1]}" ${p.category === c[1] || (c[1] === 'Riteņi & diski' && p.category === 'Riteņi un diski') ? 'selected' : ''}>${c[1]}</option>`).join('')}</select></label><label>CONDITION<input name="condition" value="${escapeHtml(p.condition || 'Labs')}"></label></div><div class="form-two"><label>PRICE (€)<input name="price" type="number" min="0" step="0.01" required value="${p.price ?? ''}"></label><label>STOCK QTY<input name="stock" type="number" min="0" value="${p.stock ?? 1}"></label></div><div class="form-two"><label>BRAND<input name="brand" value="${escapeHtml(p.brand || '')}"></label><label>MODEL<input name="model" value="${escapeHtml(p.model || '')}"></label></div><div class="form-two"><label>OEM NUMBER<input name="oem" value="${escapeHtml(p.oem || '')}"></label><label>PART CODE<input name="code" value="${escapeHtml(p.code || '')}"></label></div><div class="form-two"><label>YEAR RANGE<input name="production_year" value="${escapeHtml(p.production_year || '')}"></label><label>ENGINE<input name="engine" value="${escapeHtml(p.engine || '')}"></label></div><label>LOCATION<input name="location" value="${escapeHtml(p.location || '')}"></label><label>IMAGE URLS (one per line)<textarea name="images" rows="3">${escapeHtml((p.images?.length ? p.images : [p.image].filter(Boolean)).join('\n'))}</textarea></label><label>DESCRIPTION<textarea name="description" rows="3">${escapeHtml(p.description || '')}</textarea></label><div class="form-actions"><button class="button button-dark" type="submit">${product ? 'Save changes' : 'Add product'}</button><button class="text-button" type="button" id="admin-product-cancel">Cancel</button></div><p class="admin-note" id="admin-product-form-message"></p></form>`
}
...
async function renderAdminTab(tab) {
  ...
  if (tab === 'orders') {
    ...
    content.innerHTML = `<div class="admin-heading"><div><span class="admin-overline">FULFILLMENT</span><h2>Orders</h2></div></div><div class="admin-metrics"><div><span>Orders today</span><strong>${todayOrders.length}</strong></div><div><span>Awaiting shipment</span><strong class="warning">${orders?.filter((o) => o.status === 'pending').length || 0}</strong></div><div><span>Revenue today</span><strong>${revenueToday.toFixed(0)} €</strong></div><div><span>Total revenue (last 50)</span><strong>${totalRevenue.toFixed(0)} €</strong></div><div><span>Avg. order value</span><strong>${avgOrderValue.toFixed(0)} €</strong></div></div><div class="admin-table cols-5">${orders?.length ? `<div class="admin-table-head"><span>Order</span><span>Items</span><span>Buyer</span><span>Total</span><span>Status</span></div>${orders.map((order) => `<div class="admin-table-row"><code>#${order.id}</code><span>${escapeHtml((itemsByOrder[order.id] || []).join(', ') || '-')}</span><span>${escapeHtml(order.buyer_name)}</span><b>${Number(order.total).toFixed(2)} €</b><select class="admin-select" data-order-status="${order.id}">${['pending', 'paid', 'shipped', 'completed', 'cancelled'].map((status) => `<option ${order.status === status ? 'selected' : ''}>${status}</option>`).join('')}</select></div>`).join('')}` : '<p class="admin-note">Vēl nav neviena pasūtījuma. Norēķinu / apmaksas sistēma tiks pievienota vēlāk — tagad klienti nosūta pasūtījuma pieprasījumu, un jūs sazināties, lai vienotos par apmaksu.</p>'}</div>`
...
async function showListingDetail(id) {
  ...
  holder.innerHTML = `<div class="admin-product-form"><h3>${escapeHtml(listing.title)}</h3><div class="detail-data"><span>STATUS<strong>${listing.status}</strong></span><span>PRICE<strong>${Number(listing.price).toFixed(2)} €</strong></span><span>BRAND / MODEL<strong>${escapeHtml(listing.brand || '-')} ${escapeHtml(listing.model || '')}</strong></span><span>YEAR / ENGINE<strong>${listing.production_year || '-'} / ${escapeHtml(listing.engine || '-')}</strong></span><span>OEM NUMBER<strong>${escapeHtml(listing.oem_number || '-')}</strong></span><span>CATEGORY<strong>${escapeHtml(listing.category || '-')}</strong></span><span>CONDITION<strong>${escapeHtml(conditionNames[listing.condition] || listing.condition || '-')}</strong></span><span>LOCATION<strong>${escapeHtml(listing.location || '-')}</strong></span><span>DESCRIPTION<strong>${escapeHtml(listing.description || 'No description provided.')}</strong></span><span>SELLER<strong>${escapeHtml(profile?.display_name || 'Unknown')} · ${escapeHtml(profile?.phone || 'no phone')}${profile?.city ? ' · ' + escapeHtml(profile.city) : ''}</strong></span><span>SUBMITTED<strong>${new Date(listing.created_at).toLocaleString('lv-LV')}</strong></span></div>${imageUrls.length ? `<div class="admin-listing-images">${imageUrls.map((url) => `<img src="${url}" alt="">`).join('')}</div>` : '<p class="admin-note">No photos uploaded.</p>'}<div class="form-actions">${listing.status === 'pending' ? `<button class="approve-button" data-listing-action="active" data-listing-id="${listing.id}">Approve</button>` : ''}<button class="reject-button" data-listing-action="removed" data-listing-id="${listing.id}">Remove</button><button class="text-button" type="button" id="admin-listing-detail-close">Close</button></div></div>`
...
function galleryMarkup(images, altText) {
  ...
  return `<div class="gallery"><div class="gallery-main"><img src="${imgs[0]}" alt="${escapeHtml(altText || '')}"></div>${imgs.length > 1 ? `<button class="gallery-arrow gallery-prev" type="button" aria-label="Iepriekšējā bilde">‹</button><button class="gallery-arrow gallery-next" type="button" aria-label="Nākamā bilde">›</button><div class="gallery-dots">${imgs.map((_, i) => `<button class="gallery-dot${i === 0 ? ' active' : ''}" type="button" data-dot="${i}" aria-label="Bilde ${i + 1}"></button>`).join('')}</div>` : ''}</div>`
}
...
function loadProductDetail(id) {
  ...
  document.querySelector('#product-data').innerHTML = `<span>OEM NUMURS<strong>${escapeHtml(product.oem || '-')}</strong></span><span>RAŽOTĀJA KODS<strong>${escapeHtml(product.code || '-')}</strong></span><span>RAŽOTĀJS<strong>${escapeHtml(product.manufacturer || '-')}</strong></span><span>AUTO<strong>${escapeHtml(product.brand || '')} ${escapeHtml(product.model || '')}</strong></span><span>GADS / DZINĒJS<strong>${escapeHtml(product.production_year || '-')} / ${escapeHtml(product.engine || '-')}</strong></span><span>KATEGORIJA<strong>${escapeHtml(product.category)}</strong></span><span>STĀVOKLIS<strong>${escapeHtml(product.condition || '-')}</strong></span><span>NOLIKTAVA<strong>${escapeHtml(product.location || '-')} · Atlikums: ${stockLabel(product.stock)}</strong></span><span>SVARS / IZMĒRI<strong>${escapeHtml(product.weight || '-')} · ${escapeHtml(product.dimensions || '-')}</strong></span><span>GARANTIJA<strong>${escapeHtml(product.warranty || '-')}</strong></span>`
...
async function loadListings() {
  ...
  targets.forEach((target) => {
    target.innerHTML = `<div class="listing-head"><span>DETAĻA</span><span>AUTO</span><span>STĀVOKLIS</span><span>CENA</span><span></span></div>${rows.map((row, index) => `<a class="listing-row" data-search="${escapeHtml([row.title, row.brand, row.model].filter(Boolean).join(' ').toLowerCase())}" href="#listing-${row.id || `demo-${index + 1}`}" ><div class="listing-title-cell">${listingThumbMarkup(thumbs[row.id])}<strong>${escapeHtml(row.title)}</strong></div><span>${escapeHtml(row.brand || '-')} ${escapeHtml(row.model || '')}</span><span>${escapeHtml(conditionNames[row.condition] || row.condition || 'Nav norādīts')}</span><b>${Number(row.price).toFixed(2).replace('.', ',')} €</b><span class="listing-arrow">↗</span></a>`).join('')}`
  })
  bindListingsFilter()
  primeAndTranslate()
}
...
async function loadCategoryListings(categoryValue) {
  ...
  const conditionNames = { very_good: 'Ļoti labs', good: 'Labs', defect: 'Ar defektu' }
  const thumbs = await fetchListingThumbnails(rows)
  target.innerHTML = rows.length ? `<div class="listing-head"><span>DETAĻA</span><span>AUTO</span><span>STĀVOKLIS</span><span>CENA</span><span></span></div>${rows.map((row, index) => `<a class="listing-row" href="#listing-${row.id || `demo-${index + 1}`}" ><div class="listing-title-cell">${listingThumbMarkup(thumbs[row.id])}<strong>${escapeHtml(row.title)}</strong></div><span>${escapeHtml(row.brand || '-')} ${escapeHtml(row.model || '')}</span><span>${escapeHtml(conditionNames[row.condition] || row.condition || 'Nav norādīts')}</span><b>${Number(row.price).toFixed(2).replace('.', ',')} €</b><span class="listing-arrow">↗</span></a>`).join('')}` : '<p class="listing-loading">Šajā kategorijā pašlaik nav aktīvu sludinājumu.</p>'
  primeAndTranslate()
}
...
async function loadListingDetail(id) {
  ...
  document.querySelector('#detail-data').innerHTML = `<span>AUTO<strong>${escapeHtml(listing.brand || '-')} ${escapeHtml(listing.model || '')}</strong></span><span>STĀVOKLIS<strong>${escapeHtml(detailConditionNames[listing.condition] || listing.condition || 'Nav norādīts')}</strong></span><span>ATRAŠANĀS VIETA<strong>${escapeHtml(listing.location || 'Nav norādīta')}</strong></span>`
  const sellerHeading = document.querySelector('#detail-seller')
  if (listing.user_id) sellerHeading.innerHTML = `<a href="#seller-${listing.user_id}">${escapeHtml(listing.seller || 'Privāts pārdevējs')} ↗</a>`
...
async function loadSellerProfile(id) {
  ...
  document.querySelector('#seller-listings').innerHTML = listings?.length ? `<div class="listing-head"><span>DETAĻA</span><span>AUTO</span><span>STĀVOKLIS</span><span>CENA</span><span></span></div>${listings.map((row) => `<a class="listing-row" href="#listing-${row.id}"><div class="listing-title-cell">${listingThumbMarkup(thumbs[row.id])}<strong>${escapeHtml(row.title)}</strong></div><span>${escapeHtml(row.brand || '-')} ${escapeHtml(row.model || '')}</span><span>${escapeHtml(sellerConditionNames[row.condition] || row.condition || 'Nav norādīts')}</span><b>${Number(row.price).toFixed(2).replace('.', ',')} €</b><span class="listing-arrow">↗</span></a>`).join('')}` : '<p class="listing-loading">Šim pārdevējam pašlaik nav aktīvu sludinājumu.</p>'
  ...
  if (reviews?.length) {
    ...
    reviewsHolder.innerHTML = reviews.map((r) => `<div class="review-row"><strong>${'★'.repeat(r.rating)}${'☆'.repeat(5 - r.rating)}</strong><span>${escapeHtml(nameById[r.reviewer_id] || 'Lietotājs')} · ${new Date(r.created_at).toLocaleDateString('lv-LV')}</span>${r.comment ? `<p>${escapeHtml(r.comment)}</p>` : ''}</div>`).join('')
  } else {
...
function myListingFormMarkup(listing, existingImages) {
  ...
  return `<form class="admin-product-form" id="my-listing-form"><h3>Rediģēt sludinājumu</h3><input type="hidden" name="id" value="${l.id}"><label>NOSAUKUMS<input name="title" required value="${escapeHtml(l.title || '')}"></label><div class="form-two"><label>CENA (€)<input name="price" type="number" min="0" step="0.01" required value="${l.price ?? ''}"></label><label>OEM NUMURS<input name="oem_number" value="${escapeHtml(l.oem_number || '')}"></label></div><div class="form-two"><label>MARKA<input name="brand" value="${escapeHtml(l.brand || '')}"></label><label>MODELIS<input name="model" value="${escapeHtml(l.model || '')}"></label></div><div class="form-two"><label>GADS<input name="production_year" type="number" min="1950" max="2030" value="${l.production_year || ''}"></label><label>DZINĒJS<input name="engine" value="${escapeHtml(l.engine || '')}"></label></div><div class="form-two"><label>KATEGORIJA<select name="category">${CATEGORIES.map((c) => { const value = c[1] === 'Riteņi & diski' ? 'Riteņi un diski' : c[1]; return `<option ${l.category === value ? 'selected' : ''}>${value}</option>` }).join('')}</select></label><label>ATRAŠANĀS VIETA<input name="location" value="${escapeHtml(l.location || '')}"></label></div><label>STĀVOKLIS<select name="condition"><option value="very_good" ${l.condition === 'very_good' ? 'selected' : ''}>Ļoti labs</option><option value="good" ${l.condition === 'good' ? 'selected' : ''}>Labs</option><option value="defect" ${l.condition === 'defect' ? 'selected' : ''}>Ar defektu</option></select></label><label>APRAKSTS<textarea name="description" rows="4">${escapeHtml(l.description || '')}</textarea></label>${images.length ? `<label>ESOŠĀS BILDES<div class="admin-listing-images">${images.map((url) => `<img src="${url}" alt="">`).join('')}</div></label>` : ''}<label>PIEVIENOT BILDES<input name="images" type="file" accept="image/*" multiple></label><div class="form-actions"><button class="button button-dark" type="submit">SAGLABĀT ↗</button><button class="text-button" type="button" id="my-listing-cancel">ATCELT</button></div><p class="admin-note" id="my-listing-message"></p></form>`
}
...
async function loadMyListings(userId) {
  ...
  holder.innerHTML = `<div id="my-listing-form-holder"></div>${rows?.length ? `<div class="admin-table cols-5"><div class="admin-table-head"><span>SLUDINĀJUMS</span><span>AUTO</span><span>CENA</span><span>STATUSS</span><span>DARBĪBAS</span></div>${rows.map((row) => `<div class="admin-table-row"><strong>${escapeHtml(row.title)}</strong><span>${escapeHtml(row.brand || '-')} ${escapeHtml(row.model || '')}</span><b>${Number(row.price).toFixed(0)} €</b><span class="status-pill">${statusNames[row.status] || row.status}</span><span class="row-actions"><button class="row-action" data-my-edit-id="${row.id}">Rediģēt</button><button class="row-action" data-my-remove-id="${row.id}">Dzēst</button></span></div>`).join('')}</div>` : '<p class="listing-loading">Tev vēl nav neviena sludinājuma.</p>'}`
...
async function loadMyFavorites(userId) {
  ...
  holder.innerHTML = active.length ? `<div class="listing-head"><span>DETAĻA</span><span>AUTO</span><span>STĀVOKLIS</span><span>CENA</span><span></span></div>${active.map((row) => `<a class="listing-row" href="#listing-${row.id}"><div class="listing-title-cell">${listingThumbMarkup(thumbs[row.id])}<strong>${escapeHtml(row.title)}</strong></div><span>${escapeHtml(row.brand || '-')} ${escapeHtml(row.model || '')}</span><span>${escapeHtml(favConditionNames[row.condition] || row.condition || 'Nav norādīts')}</span><b>${Number(row.price).toFixed(2).replace('.', ',')} €</b><span class="listing-arrow">↗</span></a>`).join('')}` : '<p class="listing-loading">Tev vēl nav neviena saglabāta sludinājuma.</p>'
  primeAndTranslate()
}
